...
const DemoLoginModal=({onClose}:{onClose:()=>void})=>(
  <div style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(2,4,10,0.96)',backdropFilter:'blur(20px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:'#060A12',border:'1px solid rgba(0,242,254,0.2)',borderRadius:16,padding:36,width:380,textAlign:'center',boxShadow:'0 0 60px rgba(0,242,254,0.1)'}}>
      ...
      <button onClick={()=>supabase.auth.signInWithOAuth({provider:'github',options:{redirectTo:window.location.origin+'/nexus?mode=demo'}})} style={{width:'100%',padding:'14px',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,color:'white',fontSize:13,fontWeight:700,cursor:'pointer',marginBottom:24,display:'flex',alignItems:'center',justifyContent:'center',gap:10,fontFamily:'monospace'}}>
        ...
        <motion.div initial={{opacity:0,x:-28}} animate={{opacity:1,x:0}} transition={{duration:.8,delay:.1}} style={{flex:"1 1 440px",minWidth:0}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:14}}>
            <div className="kymia-cta-row" style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              <div>
                <button className="kymia-cta" onClick={triggerDemoModal} style={{display:"block",padding:"13px 24px",background:"rgba(0,255,136,0.12)",border:"1.5px solid rgba(0,255,136,0.5)",color:K.g,borderRadius:6,fontSize:13,cursor:"pointer",letterSpacing:".03em",fontFamily:F,fontWeight:700,transition:"all .25s"}}
        ...
        <Fade delay={.18}>
          <div className="kymia-cta-row" style={{display:"flex",gap:16,justifyContent:"center",marginBottom:16,flexWrap:"wrap",position:"relative"}}>
            {[
              ...
              <div key={b.href}>
                {'demo' in b&&b.demo
                  ?<button className="kymia-cta" onClick={triggerDemoModal} style={{display:"block",padding:"16px 36px",background:b.bg,border:b.brd,color:b.col,borderRadius:8,fontSize:14,cursor:"pointer",letterSpacing:".04em",fontFamily:F,fontWeight:700,transition:"all .25s"}}
